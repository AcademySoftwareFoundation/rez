...
def run(namespace):
    ...
    subcommands_, hidden_subcommands_ = subcommands[namespace]
    ...
    if subcommand is None:
        cmds = set(subcommands_) - set(hidden_subcommands_)
        if prefix:
            cmds = (x for x in cmds if x.startswith(prefix))
        print " ".join(cmds)

    if subcommand not in subcommands_:
        return
